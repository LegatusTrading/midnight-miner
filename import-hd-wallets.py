...
import cbor2
import requests
from pycardano import PaymentExtendedSigningKey, PaymentVerificationKey, Address, Network, sign
...
def derive_wallet_from_hd_files(hd_wallet_dir, account_num):
    ...
    # Read the .skey file
    skey_path = os.path.join(hd_wallet_dir, f"account-{account_num}", "payment", "0", "payment.skey")
    if not os.path.exists(skey_path):
        raise FileNotFoundError(f"Payment key file not found: {skey_path}")

    # Load extended signing key (handles Ed25519-BIP32 properly)
    extended_signing_key = load_extended_signing_key(skey_path)

    # Derive address from extended key
    verification_key = extended_signing_key.to_verification_key()
    ...
    vkey_bytes = bytes(verification_key.to_primitive())
    pubkey = vkey_bytes[:32].hex()  # First 32 bytes only

    return {
        'address': str(address),
        'pubkey': pubkey,
        'extended_signing_key': extended_signing_key,  # Store for CIP-8 signing
