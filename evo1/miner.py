...
from pycardano import PaymentSigningKey, PaymentVerificationKey, Address, Network, HDWallet
import cbor2
...
from rom_manager import SharedROMManager
...
class ChallengeTracker:
    ...
    def get_unsolved_challenge(self, wallet_address):
        """
        VPS6 ENHANCED: Select the best challenge for this wallet.

        - Must not be already solved by this wallet.
        - Must have > 2 hours (7200s) remaining until latest_submission.
          (VPS6 uses 2h buffer vs 120s to safely mine historical challenges)
        ...
        def find_challenge(challenges):
            ...
            for data in challenges.values():
                ...
                # VPS6: Use 2-hour buffer for historical challenge safety
                MIN_TIME_BUFFER = 7200  # 2 hours in seconds
                if time_left <= MIN_TIME_BUFFER:
...
class WalletManager:
    ...
    def generate_wallet(self):
        """Generate a new wallet from a BIP39 mnemonic (15 words)"""
        # Generate 15-word mnemonic (160 bits strength)
        mnemo = Mnemonic('english')
        mnemonic_phrase = mnemo.generate(strength=160)

        # Derive wallet from mnemonic using Cardano standard path (CIP-1852)
...
class MinerWorker:
    ...
    def __init__(self, wallet_data, worker_id, status_dict, challenge_tracker, dev_address, failed_solutions_count, failed_solutions_lock, rom_manager, donation_enabled=True, api_base="https://scavenger.prod.gd.midnighttge.io/"):
    ...
    def check_for_new_challenges_aggressive(self):
        """
        VPS6 ENHANCEMENT: Aggressively check for new challenges.

        This method is called more frequently than the default behavior
        to ensure we catch new challenges as soon as they appear.

        This allows us to build up a database of challenges over time
        and always mine the easiest available ones.
        """
        api_challenge = self.get_current_challenge()
        if api_challenge:
            is_new = self.challenge_tracker.register_challenge(api_challenge)
            if is_new:
                self.logger.info(f"Worker {self.worker_id}: ✨ Discovered NEW challenge {api_challenge['challenge_id']} (difficulty: {api_challenge['difficulty'][:8]})")
                return True
        return False
    ...
    def run(self):
        ...
        self.logger.info(f"Worker {self.worker_id} ({self.short_addr}): Starting mining worker (EVO1 with SharedROM)...")
        ...
        while True:
            try:
                ...
                # Get or build ROM for this challenge using SharedROMManager
                no_pre_mine = challenge["no_pre_mine"]
                self.update_status(current_challenge=f'Getting ROM')
                rom = self.rom_manager.get_or_build_rom(no_pre_mine, challenge_id)
...
def worker_process(wallet_data, worker_id, status_dict, challenges_file, dev_address, failed_solutions_count, failed_solutions_lock, rom_manager, donation_enabled=True):
    """Process entry point for worker"""
    try:
        setup_logging()
        challenge_tracker = ChallengeTracker(challenges_file)
        worker = MinerWorker(wallet_data, worker_id, status_dict, challenge_tracker, dev_address, failed_solutions_count, failed_solutions_lock, rom_manager, donation_enabled=donation_enabled)
        worker.run()
    except Exception as e:
        logger = logging.getLogger('midnight_miner')
...
def main():
    ...
    print("="*70)
    print(f"MIDNIGHT MINER EVO1 - v{VERSION}")
    print("="*70)
    print()

    logger.info("="*70)
    logger.info("Midnight Miner EVO1 starting up (SharedROM architecture)...")
    logger.info("="*70)

    num_workers = 20  # EVO1: Default to 20 workers (vs 8 in base)
    ...
    # EVO1: Create SharedROMManager for memory-efficient ROM caching
    print(f"✓ Creating SharedROMManager (enables {num_workers} workers with shared ROM)")
    logger.info(f"Creating SharedROMManager for {num_workers} workers")
    rom_manager = SharedROMManager(ashmaize_py)
    ...
    def spawn_worker(worker_id):
        ...
        with worker_lock:
            ...
            p = Process(target=worker_process, args=(wallet, worker_id, status_dict, challenges_file, dev_address, failed_solutions_count, failed_solutions_lock, rom_manager, donation_enabled))
            p.start()
